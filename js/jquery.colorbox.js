	...
	pub.colorboxSetup = function(){
		...
		$(document)
			.on('cbox_open', function(){
				...
				//add a dummy history state to close colorbox on back-button click
			...
			//prevent colorbox close when we're focused on a contained form field
			...
			//close when clicking the wrapper as it has a margin which reveals some of the overlay
			/*.on('click', '#cboxWrapper', function(e){
				//check it's not a child item that was clicked
				if ($(e.target).is('#cboxWrapper')){
					$.colorbox.close();
				}
			});*/
			
		$(window)
			//on back-button click, close as there is no hash
			.on('hashchange', function(){
				if (!window.location.hash.match(hash)){
					if (body.hasClass('colorbox') && !body.hasClass('input-focus')){
						$.colorbox.close();
					}
				}
			})
			...
			.on('resize', function(){
				HELP.onEventFinish(function(){
					if (body.hasClass('colorbox') && $.isset($.colorbox)){
						var vw = HELP.viewportWidth(),
							cbox = $('#colorbox'),
							w = cbox.data('options').width ? cbox.data('options').width : null,
							h = cbox.data('options').height ? cbox.data('options').height : null;
